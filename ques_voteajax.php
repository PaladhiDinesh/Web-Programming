...
<?php 
	if($USERID != "undefined"){
		...
		$vote_select_query = "SELECT user_id,question_id,votes FROM votes_table WHERE user_id=$user_id and question_id=$ques_id";
		...
		if ($select_result->num_rows ==0){
			...
			if($vote==1){
				if ($que_vote_row['votes']==1){
					...
					$vote_update_query= "UPDATE votes_table SET votes = votes+1 WHERE user_id=$user_id and question_id=$ques_id";
